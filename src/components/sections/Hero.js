...
import { playfairDisplay, playfairDisplaySC } from "@/styles/fonts";
...
const StyledFirstCircle = styled.div`
  ...
  background: linear-gradient(180deg, #f85b3b, rgba(217, 217, 217, 0) 50%);
  filter: blur(25px);
...
export default function Hero({ workSectionScroll }) {
  ...
  const firstCircleStyle = {
    left: `${Math.min(35 + 30 * workSectionScroll, 50)}%`,
    maskPosition: `${Math.max(100 - workSectionScroll * 100, 50)}% 50%`,
  };

  const secondCircleStyle = {
    left: `${Math.max(65 - 30 * workSectionScroll, 50)}%`,
    maskPosition: `${Math.min(0 + workSectionScroll * 100, 50)}% 50%`,
  };

  const textAnimationStyle = {
    letterSpacing: `${2.5 - 3 * workSectionScroll}vw`,
    opacity: `${workSectionScroll >= 0.5 ? 0 : 1}`,
  };
  ...
  const captionAnimationStyle = {
    opacity: `${1 - workSectionScroll * 3}`,
  };

  const scrollButtonStyle = {
    display: `${workSectionScroll !== 0 ? "none" : "block"}`,
  };
  ...
  return (
    <StyledHeroSection ref={heroSectionRef} style={sectionStyle}>
      ...
      <StyledHeroContainer className={playfairDisplaySC.className}>
        <StyledHeaderPart
          style={isMobile ? mobileTextAnimationStyle : textAnimationStyle}
        >
          <h3>Hello</h3>
        </StyledHeaderPart>
        <StyledHeaderPart
        ...
        >
          ...
          <StyledSecondWord onClick={() => helloThere.current.play()}>
            There
          </StyledSecondWord>
          <audio src={"hellothere.mp3"} ref={helloThere} />
        </StyledHeaderPart>
        <StyledCaptions
          style={isMobile ? mobileTextAnimationStyle : captionAnimationStyle}
